...
// Khai báo chân cắm trên arduino
int RedLed    = 15;                 
int GreenLed  = 16;              
int Blueled   = 17;
int lock      = 9;
...

{
  Serial.begin(9600);                                    
  SPI.begin();                                            
  mfrc522.PCD_Init();       // Thiết lập đầu ra các thiệt bị                              
  ...
  lcd.setCursor(0, 1);
  lcd.print("THPT Mac Dinh Chi smart home");
  delay(5000);
  lcd.setCursor(1, 3);
  lcd.print(" Cửa khóa :-   ");
  lcd.setCursor(13, 3);
...
// Thiết lập thẻ RIFD
  ...
  { 
    Reset = 1;
    Serial.println("Tatquyentruycapthe");
  }
        ...
        mfrc522.PICC_ReadCardSerial())
  {
...

    {
      ...
      PinCodeCounter = 0;                               
      Timer =  millis();                                 
      Serial.println("Batquyentruycapthe");                     
      delay (200);                                       
    }
    if
  ...
  if (Code6Correct == 1 && RightCard == 1)                   
  {
    ...
    lcd.clear();
    lcd.setCursor(6, 1);
    lcd.print("Xin vui long ghe tham ");
    lcd.setCursor(8, 2);
    lcd.print("Lan sau");
    delay(5000);
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Chao mung den voi");
    lcd.setCursor(0, 1);
    lcd.print("THPT Mac Dinh Chi smarthome");
    lcd.setCursor(1, 3);
    lcd.print(" Cua khoa :-   ");
    lcd.setCursor(13, 3);
    lcd.print("CLOSE");

    Serial.println("Ma chinh xac");                      
    Reset = 1;                                            
  }
  ...
  if ((WrongPinCode == 1) || (millis() - Timer > 7000 && RightCard == 1))    
  {
    ...
    Serial.println("Ma sai hoac het thoi gian");                           
    Reset = 1;                                                              
  }
